...
from src.modeling_roberta import RobertaForTokenClassification_v2
from src.modeling_bert import BERTForTokenClassification_v2
from src.data_utils import load_and_cache_examples, get_labels
from src.model_utils import mt_update, get_mt_loss, opt_grad
...
def main(targets):
    ...
    if "test" in targets:
        ...
        parser.add_argument(
            "--data_dir",
            default="../dataset/webpage_distant",
            type=str,
            required=False,
            help="The input data dir. Should contain the training files for the CoNLL-2003 NER task.",
        )
    ...
    # Read in Config file and parse the arguments
    config_dict = json.load(open('./config/config.json'))
    for config in config_dict:
        if (config != "data_dir") and (config != "output_dir"):
            exec("args.{} = {}".format(config, config_dict[config]))
        elif config == "data_dir":
            args.data_dir = config_dict[config]
        else:
            args.output_dir = config_dict[config]
